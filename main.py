...
def set_user_state(user_id, state, sub_state=None):
    """Устанавливает состояние пользователя"""
    user_states[user_id] = state
    if sub_state:
        user_states[f"{user_id}_sub"] = sub_state
...
def get_user_substate(user_id):
    """Получает текущий подмодуль пользователя"""
    return user_states.get(f"{user_id}_sub", '')
...
def handle_message(event):
    ...
    # Обработка модулей
    if current_state == 'movement_tasks':
        try:
    ...
    elif current_state == 'concentration_tasks':
        try:
    ...
    elif current_state == 'percentage_tasks':
        try:
    ...
    elif current_state == 'text_tasks_menu':
        handle_text_tasks_menu(user_id, text)

    elif current_state in ['main_menu', '']:
...
def handle_text_tasks_menu(user_id, text):
    ...
    if text == BUTTONS["movement"]:  # "🚗 Движение"
        show_movement_main_menu(vk, user_id)
    elif text == BUTTONS["work"]:  # "🛠️ Работа"
        show_work_main_menu(vk, user_id)
    elif text == BUTTONS["concentration"]:  # "🧪 Концентрация"
        show_concentration_main_menu(vk, user_id)
    elif text == BUTTONS["percentage"]:  # "📊 Проценты"
